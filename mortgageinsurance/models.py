...
class Monthly(models.Model):
    ...
    insurer = models.CharField(
        max_length=200,
        help_text='Abbreviation of the mortgage insurer')
    min_ltv = models.DecimalField(
        max_digits=6, decimal_places=3,
        help_text='Minimum loan to value ratio')
    ...
    def __unicode__(self):
        return (u'{} ltv: ({} {}) fico: ({} {}) loan term: ({}) {} '
                'loan amount: ({} {}) Premium: {}').format(
                    self.insurer,
                    self.min_ltv,
                    self.max_ltv,
                    self.min_fico,
    ...
    def get_avg_premium(params_data):
        ...
        ltv = (Decimal(params_data['loan_amount']) /
               Decimal(params_data['price']) * 100).quantize(
        ...
        if params_data['loan_type'] not in (Monthly.VA, Monthly.VA_HB):
            ...
            result = Monthly.objects.filter(
                ...
                # Currently ARM will use FIXED data (3/1 will not get
                # to this point)
                # Q(pmt_type=params_data['rate_structure']) &
                Q(min_loan_amt__lte=params_data['loan_amount']) &
                Q(max_loan_amt__gte=params_data['loan_amount'])
            ).aggregate(Avg('premium'))

            avg_premium = (float('nan') if result['premium__avg'] is None else
                           round(result['premium__avg'], 3))
...
class Upfront(models.Model):
    ...
    loan_type = models.CharField(
        max_length=12,
        choices=Monthly.LOAN_TYPE_CHOICES,
        help_text='Loan Type')
    va_status = models.CharField(
        max_length=12,
        choices=VA_STATUS_CHOICES,
        blank=True,
        help_text='VA Status')
    va_first_use = models.NullBooleanField(
        blank=True,
        help_text='VA First Time Use')
    min_ltv = models.DecimalField(
        max_digits=6,
        decimal_places=3,
        help_text='Minimum loan to value ratio')
    ...
    def __unicode__(self):
        return (u'{} va status: ({}), va first use? ({}) '
                'ltv: ({} {}) Premium: {}').format(
                self.loan_type,
                self.va_status,
                self.va_first_use,
    ...
    def get_premium(params_data):
        ...
        if params_data['loan_type'] in (Monthly.FHA, Monthly.FHA_HB):
            result = Upfront.objects.filter(
                loan_type=Monthly.FHA,
                min_ltv__lte=ltv,
                max_ltv__gte=ltv,
                ).first()
        ...
        premium = (float('nan') if result == {} or
                   result is None or
                   result.premium is None else
                   round(result.premium, 3))
