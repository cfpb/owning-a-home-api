...
class ParamsSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        attrs['min_ltv'] = Decimal("%f" % (attrs['loan_amount'] / attrs['price'] * 100)).quantize(Decimal('.001'))
        ...
        if attrs['minfico'] > attrs['maxfico']:
           attrs['minfico'], attrs['maxfico'] = attrs['maxfico'], attrs['minfico']
    ...
    def validate_lock(self, attrs, source):
        ...
        if attrs[source] not in (30, 45, 60):
            raise serializers.ValidationError("lock needs to be 30, 45, or 60.")
    ...
    def validate_loan_term(self, attrs, source):
        ...
        if attrs[source] not in (15, 30):
            raise serializers.ValidationError("loan_term needs to be 15 or 30.")
        return attrs
